...
{ 
  ...
  if (QSPI_AutoPollingMemReady(W25Qxxx_SUBSECTOR_ERASE_MAX_TIME) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
* @param  ReadAddr: Read start address (Page)
...
* @param  WriteAddr: Write start address (Page)
...
uint8_t BSP_QSPI_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  {
    current_addr += W25Qxxx_PAGE_SIZE;
  }
...
* @retval QSPI memory status
*/
uint8_t BSP_QSPI_Erase_Sector_4KB(uint32_t SectorAddress)
{
  QSPI_CommandTypeDef s_command;
  
  /* Initialize the erase command */
  s_command.InstructionMode   = QSPI_INSTRUCTION_1_LINE;
  s_command.Instruction       = SECTOR_4KB_ERASE_CMD;
  s_command.AddressMode       = QSPI_ADDRESS_1_LINE;
  s_command.AddressSize       = QSPI_ADDRESS_24_BITS;
...
* @retval QSPI memory status
*/
uint8_t BSP_QSPI_Erase_Block_32KB(uint32_t BlockAddress)
{
  QSPI_CommandTypeDef s_command;
  
  /* Initialize the erase command */
  s_command.InstructionMode   = QSPI_INSTRUCTION_1_LINE;
  s_command.Instruction       = BLOCK_32KB_ERASE_CMD;
  s_command.AddressMode       = QSPI_ADDRESS_1_LINE;
  s_command.AddressSize       = QSPI_ADDRESS_24_BITS;
...
* @retval QSPI memory status
*/
uint8_t BSP_QSPI_Erase_Block_64KB(uint32_t BlockAddress)
{
  QSPI_CommandTypeDef s_command;
  
  /* Initialize the erase command */
  s_command.InstructionMode   = QSPI_INSTRUCTION_1_LINE;
  s_command.Instruction       = BLOCK_64KB_ERASE_CMD;
  s_command.AddressMode       = QSPI_ADDRESS_1_LINE;
  s_command.AddressSize       = QSPI_ADDRESS_24_BITS;
  s_command.Address           = BlockAddress;
  s_command.AlternateByteMode = QSPI_ALTERNATE_BYTES_NONE;
...
uint8_t BSP_QSPI_Erase_Chip(void)
{
  ...
  /* Configure automatic polling mode to wait for end of erase */  
  if (QSPI_AutoPollingMemReady(W25Qxxx_BULK_ERASE_MAX_TIME) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
uint8_t BSP_QSPI_GetInfo(QSPI_Info* pInfo)
{
  ...
  pInfo->FlashSize          = W25Qxxx_FLASH_SIZE;
  pInfo->EraseSectorSize    = W25Qxxx_SUBSECTOR_SIZE;
  pInfo->EraseSectorsNumber = (W25Qxxx_FLASH_SIZE/W25Qxxx_SUBSECTOR_SIZE);
  pInfo->ProgPageSize       = W25Qxxx_PAGE_SIZE;
  pInfo->ProgPagesNumber    = (W25Qxxx_FLASH_SIZE/W25Qxxx_PAGE_SIZE);
...
uint8_t BSP_QSPI_MemoryMappedMode(void)
{
  ...
  s_command.AlternateBytesSize = QSPI_ALTERNATE_BYTES_8_BITS;
  s_command.DataMode          = QSPI_DATA_4_LINES;
  s_command.DummyCycles       = W25Qxxx_DUMMY_CYCLES_READ_QUAD;
  s_command.DdrMode           = QSPI_DDR_MODE_DISABLE;
  s_command.DdrHoldHalfCycle  = QSPI_DDR_HHC_ANALOG_DELAY;
  s_command.SIOOMode          = QSPI_SIOO_INST_EVERY_CMD;
...
static uint8_t QSPI_WriteEnable()
{
  ...
  s_config.Match           = W25Qxxx_FSR_WREN;
  s_config.Mask            = W25Qxxx_FSR_WREN;
...
static uint8_t QSPI_AutoPollingMemReady(uint32_t Timeout)
{
  ...
  s_config.Match           = 0x00;
  s_config.Mask            = W25Qxxx_FSR_BUSY;
  s_config.MatchMode       = QSPI_MATCH_MODE_AND;
  s_config.StatusBytesSize = 1;
  s_config.Interval        = 0x10;
  s_config.AutomaticStop   = QSPI_AUTOMATIC_STOP_ENABLE;
