...
u8 SD_ReceiveData(u8 *data, u16 len, u8 release)
{
    ...
    do
    {
        r1 = SPI_ReadWriteByte(0xFF);
        retry++;
        if(retry>4000)  //2000次等待后没有应答，退出报错
        {
            SD_CS_DISABLE();
            return 1;
        }
...
u8 SD_GetCSD(u8 *csd_data)
{
    ...
    //接收16个字节的数据
    if(SD_ReceiveData(csd_data, 16, RELEASE)){
		return 1;
	}
...
static rt_size_t rt_sdcard_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
    ...
    while(retry)
    {
            ...
            ((rt_uint32_t)buffer > 0x20080000))
        {
            ...
            {
                status = SD_ReadSingleBlock((part.offset + index) * SECTOR_SIZE + pos*SECTOR_SIZE,
                    (uint8_t*)_sdcard_buffer);
        ...
        else
        {
            if (nr == 1)
            {
                status = SD_ReadSingleBlock(part.offset * SECTOR_SIZE + pos*SECTOR_SIZE,
                    (uint8_t*)buffer);
            }
            else
            {
                status = SD_ReadMultiBlock(part.offset * SECTOR_SIZE + pos*SECTOR_SIZE,
                    (uint8_t*)buffer,  nr);
            }
        }
...
static rt_size_t rt_sdcard_write (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
        ...
        ((rt_uint32_t)buffer > 0x20080000))
	{
        ...
        for (index = 0; index < nr; index ++)
        {
            ...
            status = SD_WriteSingleBlock((part.offset + index) * SECTOR_SIZE + pos*SECTOR_SIZE,
                (uint8_t*)_sdcard_buffer);
	...
	else
	{
        if (nr == 1)
        {
            status = SD_WriteSingleBlock(part.offset * SECTOR_SIZE + pos*SECTOR_SIZE,
                (uint8_t*)buffer);
        }
        else
        {
            status = SD_WriteMultiBlock(part.offset * SECTOR_SIZE + pos*SECTOR_SIZE,
                (uint8_t*)buffer, nr);
        }
	}
...
////        geometry->block_size = 8*512;//SDCardInfo.CardBlockSize;
////		if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
////			geometry->sector_count = 8*512;//(SDCardInfo.SD_csd.DeviceSize + 1)  * 1024;
////		else
////        	geometry->sector_count = 8*512;//SDCardInfo.CardCapacity/SDCardInfo.CardBlockSize;
////    }
//
//    return RT_EOK;
//}
...
void rt_hw_sdcard_init()
{
    ...
    if (SD_Init() == 0)
	{
		...
		status = SD_ReadSingleBlock(0, (uint8_t*)sector);
		if (status == 0)
		{
...
//修改		sdcard_device.control = rt_sdcard_control;

		/* no private */
		sdcard_device.user_data = 0;//&SDCardInfo;
